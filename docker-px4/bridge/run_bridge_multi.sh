...
docker run -it                                  `# Run the container interactively with a pseudo-TTY` \
    ...
    --volume="${DOCKER_PX4_PATH}/bridge/docker-scripts-multi:/docker-scripts"   `# Mount a volume from the host to the container (host_path:container_path)` \
    --volume="${DOCKER_PX4_PATH}/bridge/docker-scripts-multi/config-files/px4_ros_api/launch:/root/px4_ros_com_ros2/src/px4_ros_api/launch" `# Mount a volume from the host to the container (host_path:container_path)` \
    --volume="${DOCKER_PX4_PATH}/bridge/docker-scripts-multi/config-files/px4_ros_api/params:/root/px4_ros_com_ros2/src/px4_ros_api/params" `# Mount a volume from the host to the container (host_path:container_path)` \
    --env-file "${DOCKER_PX4_PATH}/bridge/docker-scripts-multi/env.sh"  `# Pass environment variables inside the container` \
